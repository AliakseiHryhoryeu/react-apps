...
export default function AimGame() {
	...
	function setColor(elem: HTMLDivElement) {
		...
		return (
			...
			(elem.style.boxShadow = `0 0 10px ${color}, 0 0 25px ${color}`)
		)
	}
	...
	return (
		<section className={styles.gameContent}>
			...
			>
				...
				<a
					onClick={() => {
						setWelcomeScreenActive(!welcomeScreenActive)
					}}
			...
			<div
				className={classNames(styles.screen, timeScreenActive ? styles.up : '')}
			>
					...
					{gameTime.map(({ id, time }) => (
						<>
							<li
								...
								onClick={() => {
									setState(prevState => {
										return { ...prevState, count: time }
									})
									setTimeScreenActive(!timeScreenActive)
									startGame(time)
								}}
							>
								<button className={styles.timeBtn}>{time} sec</button>
							</li>
						</>
					))}
				</ul>
			...
			<div className={styles.screen}>
				<h3 className={styles.screenTitle}>
					<span
						ref={timeEl}
						className={classNames(state.count <= 10 ? styles.colorRed : '')}
					>
						{state.count}{' '}
					</span>
					seconds left
				...
				<div ref={board} className={styles.board}>
					<div
						className={classNames(state.finishGame ? '' : styles.displayNone)}
					>
						<h1>
							Your score: <span className={styles.primary}>{state.score}</span>
						</h1>
						<br />
